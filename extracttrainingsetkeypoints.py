...
"""
 @Author: Sailor
 @FileName: extracttrainingsetkeypoints.py
 @DateTime: 2023.01.31 14:21
 @SoftWare: PyCharm
"""
import os
import sys
...
class BootstrapHelper(object):
    ...
    def __init__(self,
        ...
        self._csvs_out_folder = csvs_out_folder     # 通过样本图像提取的特征值作为训练集 输出为CSV文件
    ...
    def bootstrap(self, per_pose_class_limit=None):
        ...
        for pose_class_name in self._pose_class_names:
            ...
            with open(csv_out_path, 'w', newline='') as csv_out_file:
                ...
                # 引导提取每个图像。用tqdm包裹iterable对象实现进度条，此处为训练集图片list
                for image_name in tqdm.tqdm(image_names):
                    # Load image.
                    input_frame = cv2.imread(os.path.join(images_in_folder, image_name))
                    input_frame = cv2.cvtColor(input_frame, cv2.COLOR_BGR2RGB)

                    # 把图片放入姿势检测模型中并返回33个landmarks的x,y,z坐标.
                    with mp_pose.Pose() as pose_tracker:
                        result = pose_tracker.process(image=input_frame)
                        pose_landmarks = result.pose_landmarks

                    # 使用保存图像中检测的姿势（如果检测到姿势）。保存检测结果至output_frame中
    ...
    def align_images_and_csvs(self, print_removed_items=False):
        """
        确保图像文件夹和 CSV 具有相同的样本。
        仅在图像文件夹和 CSV 中保留样本的交集。
        """
        for pose_class_name in self._pose_class_names:
