...
class PoseClassifier(object):
    ...
    def _load_pose_samples(self,
        ...
        for file_name in file_names:
            ...
            # Parse CSV.
            # 重新解析CSV文件为PoseSample组成的list，每一个PoseSample对象为一个训练图片的相关信息.结构为：
            #    # 图片名称 （str）；
            #    # landmarks （np.array）；
            #    # 姿势类别名称（str）；
            #    # 姿势的embedding对象（pose_embedder）
    ...
    def find_pose_sample_outliers(self):
        ...
        for sample in self._pose_samples:
            ...
            # 如果最近的姿势具有不同的类别或多个姿势类别被检测为最近（最后的结果可能即是A pose 又是B pose），则样本是异常值。
    ...
    def __call__(self, pose_landmarks):
        ...
        Returns:
          Dictionary with count of nearest pose samples from the database.含数据库中最近姿势样本计数的字典
            Sample:
            {
              'pushups_down': 8,
              'pushups_up': 2,
        ...
        """
        # 检查提供的姿势和目标姿势是否具有相同的landmarks数据结构. （33，3）
